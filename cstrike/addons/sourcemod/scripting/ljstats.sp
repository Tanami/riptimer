...
PlayerJump(client, JUMP_TYPE:JumpType2 = JT_LONGJUMP)
{
	...
	if(g_PlayerStates[client][JumpType] == JT_LONGJUMP)
	{
		...
		{
			// PrintToChat(client, "Illegal jump: prestrafe > 280.");
		...
		if(!g_bLJScoutStats && (g_fMaxspeed > 250.0 && GetEntPropFloat(client, Prop_Data, "m_flMaxspeed") > 250.0))
		{
			...
			// int accel = FindConVar( "sv_airaccelerate" ).IntValue;
			// PrintToChatAll("%d", accel);
...
PlayerLand(client)
{
	...
	QueryClientConVar(client, "sv_airaccelerate", ConVarQueryFinished:checkaccel);

	// int accel = FindConVar( "sv_airaccelerate" ).IntValue;
	// PrintToChat(client, "%d", accel);
	// if (accel > 100) {
	// }
	// else {
	// }
...
public checkaccel(QueryCookie:cookie, client, ConVarQueryResult:result, const String:cvarName1[], const String:cvarValue1[])
{
	if(IsClientConnected(client))
	{
		if(StringToInt(cvarValue1) > 100 && g_PlayerStates[client][fJumpDistance] > 260)
		{
			PrintToChat(client, "Sorry, but you need to be playing Legit (100AA) for this jump to be recorded.");
		}
		else
		{
			UpdatePersonalBest(client);
