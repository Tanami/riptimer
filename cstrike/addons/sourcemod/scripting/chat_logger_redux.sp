...
public void OnPluginStart()
{
	...
	ConVars[2] = CreateConVar("sm_chat_log_timeformat", "%d %b %H:%M:%S %Y |", "Format for time stamp in files", FCVAR_NOTIFY);
...
public void OnClientSayCommand_Post(int client, const char[] command, const char[] sArgs)
{
	...
	if (client > 0 && client <= MaxClients)
	{
		...
		{
			Format(sFormat, sizeof(sFormat), "%s %s %s", sTime, sAuth, sName);
		}
	}
	else if (sv_bConsole)
	{
