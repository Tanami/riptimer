...
//		{
			FormatEx(sQuery, 512,
				"SELECT COUNT(*) FROM playertimes a JOIN (SELECT MIN(time) time, map FROM playertimes WHERE style in (3,0) and track = 0 GROUP by map, style, track) b ON a.time = b.time AND a.map = b.map AND style in (3,0) and track = 0 WHERE auth = %d;",
				iSteamID);
//		}
